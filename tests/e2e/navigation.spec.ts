...
test.describe("Sidebar navigation", () => {
  ...
  test("Jobs page renders without error", async ({ page }) => {
    await page.goto("/jobs");
    await expect(page.locator("h1")).toContainText("All Jobs");
    await expect(page.locator("text=Application error")).not.toBeVisible();
  });

  test("System page renders without error", async ({ page }) => {
    await page.goto("/system");
    await expect(page.locator("h1")).toContainText("System");
    await expect(page.locator("text=Application error")).not.toBeVisible();
  });

  test("Memory page renders without error", async ({ page }) => {
  ...
  test("Active nav highlighted on agents page", async ({ page }) => {
    await page.goto("/agents");
    const agentsLink = page.locator('a[href="/agents"]');
    await expect(agentsLink).toHaveCSS("color", "rgb(167, 139, 250)");
  });

  test("Active nav highlighted on jobs page", async ({ page }) => {
    await page.goto("/jobs");
